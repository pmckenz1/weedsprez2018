...
sdm_function <- function(directory.path,coords = NULL,species.name,climate.map.path = NULL,threshold = "balanced",
                         raster.location = NULL, do.plot = TRUE,maxent.path = "maxent.jar",updateProgress = NULL) {
...
if (is.function(updateProgress)) {
    updateProgress(message = "Finished getting climate data.")
}
...
if (is.function(updateProgress)) {
    updateProgress(message = "Getting occurrence data...", value = .3)
}
...
if (is.function(updateProgress)) {
    updateProgress(message = "Finished getting occurrence data.", value = .35)
}
...
if (is.null(raster.location)) {
  ...
  if (is.function(updateProgress)) {
      updateProgress(message = "Writing raster inputs.", value = .55)
  }
...
if (is.function(updateProgress)) {
    updateProgress(message = "Starting maxent.", value = .75)
}
...
if (do.plot == TRUE) {
  if (is.function(updateProgress)) {
        updateProgress(message = "Reading maxent outputs.", value = .95)
  }
  ...
  } #else if (threshold == "none") {return(plot(crop(maxent_output,ext)))}
  #m <- c(0,mapthreshold,0,mapthreshold,1,1)
  #rclmat <- matrix(m, ncol=3, byrow=TRUE)
  #rc <- reclassify(maxent_output, rclmat)
  plot(crop(maxent_output,ext))
  #print("......Finished")
